...
<form method="GET" action="prodetails.php"><table>  
      ...
      if (!$conn) {
		...
		$value = $_GET ['idclient']; 
		$query = "SELECT * FROM produit WHERE type_prod='chocolat'";
		...
		if($row = mysqli_fetch_assoc($result)){
			$nom=$row['nom_pro'];
			$imagepath=$row['image'];	   
		echo'<style>td{width:300px;height:300px;}</style>';
        echo '<td>'.'<img src="'.$imagepath.'"height="200" width="150"/></br>'.$row['nom_pro'].
